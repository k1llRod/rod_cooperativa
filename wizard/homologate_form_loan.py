...
class HomolagateForm(models.TransientModel):
    ...
    def compute_period(self):
        for record in self:
            ...
            record.count_period = len(
                self.env['nominal.relationship.mindef.loan'].search([('period_process', '=', period)]))
            record.count_register_reconciled = len(
                self.env['nominal.relationship.mindef.loan'].search([('period_process', '=', False)]))
    ...
    def homolagate(self):
        ...
        for rec in filing_cabinet_ids:
            ...
            rec.state = 'draft'
        context = {'default_message': 'Se han homologado ' + str(
            len(filing_cabinet_ids)) + ' del periodo ' + period + ' con éxito.'}
        return {
