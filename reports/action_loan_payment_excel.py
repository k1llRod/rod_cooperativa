...
class LoanPaymentXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, payments):
        ...
        headers = ['No.', 'CODIGO', 'GRADO', 'AP.PATERNO', 'AP.MATERNO', 'NOMBRES', 'PERIODO', 'D/MINDEF Bs']
        ...
        for rec in records:
            ...
            sheet.write(row, 1, getattr(partner, 'code_contact', '') or '', fmt_text)
            ...
            sheet.write(row, 6, getattr(rec, 'period', '') or '', fmt_text)
            sheet.write(row, 7, getattr(rec, 'amount_total_bs', '') or '', fmt_text)
            ...
            if rec.flag_collect_guarantors == True:
                ...
                sheet.write(row, 6, getattr(rec, 'period', '') or '', fmt_yellow)
